...
import { getDaysGrid, getWeekdays, idMaker, isCurrentMonth, parseDateString, isSameDate, getDateFormat, maskInput, } from "./helpers";
import { DEFAULT_ICON, MONTH_DECREMENT_ICON, MONTH_INCREMENT_ICON, YEAR_DECREMENT_ICON, YEAR_INCREMENT_ICON, } from "./icons";
import { DatePickerContainer, HintContainer, InputButton, InputWrapper, HintText, WeekdayCell, CalendarHeader, InputOutline, InputLabel, InputField, Input, Overlay, CalendarButtonGroup, CalendarCell, CalendarGrid, CalendarPopup, } from "./StyledComponents";
const DEFAULT_PROPS = {
    ref: null,
    value: null,
    color: "#009898",
...
export function DatePicker(props) {
    ...
    const getDateSchema = () => {
        ...
        return dateFormat
            ...
            .replace(props.delimiter || "/", ""); // YMD | }MDY | DMY
    };
    ...
    const getCellWeekday = el => {
        ...
        if (!day)
            console.error("getCellWeekday errored out!");
        return grid.findIndex(o => o.day === day &&
            o.date.getTime() ===
                new Date(shownDate().getFullYear(), shownDate().getMonth(), day).getTime());
    };
    const daysGrid = (date) => {
        let d = date || new Date();
    ...
    function handleInputKeyDown(e) {
        ...
        if (e.code === "Enter") {
            if (props.disabled || props.calendarDisabled)
                return;
            setIsOpen(!isOpen());
        }
        if (e.code === "ArrowDown") {
    ...
    function handleCellKeyDown(e, d) {
        const cellIndex = cellsRefs.findIndex(ref => ref.firstChild.textContent === d.day.toString());
        ...
        switch (e.code) {
            ...
            case "ArrowRight": {
                if (cellIndex + 1 >= +lastCell.firstChild.textContent)
                    return;
                if (d.weekday < 6)
                    cellsRefs[cellIndex + 1].firstChild.focus();
                break;
            }
            case "ArrowDown": {
    ...
    const monthDecrement = e => {
        cellsRefs = [];
        const currentDay = shownDate().getDate(), currentMonth = shownDate().getMonth(), currentYear = shownDate().getFullYear();
        const prevMonth = currentMonth === 0 ? 11 : currentMonth - 1;
        const prevDate = new Date(prevMonth === 11 ? currentYear - 1 : currentYear, prevMonth, currentDay);
    ...
    const monthIncrement = e => {
        ...
        // prettier-ignore
        new Date(currentMonth === 11 ? currentYear + 1 : currentYear, nextMonth, 1).getTime() - 1000 * 60).getDate();
    ...
    return (<DatePickerContainer ref={props.ref} theme={props.theme}>
			<InputField theme={props.theme} width={props.inputWidth} onClick={e => {
            if (props.inputDisabled) {
                setIsOpen(true);
                cellsRefs[0].firstChild.focus();
            }
            inputRef.focus();
				...
				<InputWrapper theme={props.theme} cursor={props.disabled || props.inputDisabled ? "default" : "text"}>
					{/* INPUT LABEL */}
					<InputLabel ref={labelRef} color={props.color} theme={props.theme} isFocused={inputFocused()} isDisabled={props.disabled ||
            (props.inputDisabled && inputRef.value.length === 0)}>
						{props.label}
					</InputLabel>

					{/* TEXT INPUT */}
					<Input ref={inputRef} type="text" theme={props.theme} placeholder={props.placeholder} onFocus={e => {
            clearTimeout(timeout);
            labelRef.classList.add("is-focused");
            outlineRef.classList.add("is-focused");
            setInputFocused(true);
        ...
        }} onKeyDown={handleInputKeyDown} onInput={handleInput} onChange={props.onChange} disabled={props.disabled || props.inputDisabled}/>

					{/* INPUT BUTTON */}
					<InputButton theme={props.theme} ref={iconBtnRef} disabled={props.disabled || props.calendarDisabled} onClick={e => {
            setIsOpen(true);
        }} onKeyDown={e => {
            if (e.code === "Enter") {
            ...
            if (e.code === "ArrowDown") {
				...
				<InputOutline ref={outlineRef} class="outline" theme={props.theme} isFocused={inputFocused()} color={props.color}/>
			</InputField>
					...
					<CalendarPopup ref={calendarPopupRef} theme={props.theme}>
						...
						<CalendarHeader theme={props.theme}>
							<CalendarButtonGroup theme={props.theme}>
								...
								<Show when={props.showYearButtons}>
									<button ref={yearDecrBtn} onClick={yearDecrement} onKeyDown={handleYearDecrKeyDown}>
										<YEAR_DECREMENT_ICON />
									</button>
								</Show>
								<button ref={monthDecrBtn} onClick={monthDecrement} onKeyDown={handleMonthDecrKeyDown}>
							...
							<CalendarButtonGroup theme={props.theme}>
								<button ref={monthIncrBtn} onClick={monthIncrement} onKeyDown={handleMonthIncrKeyDown}>
									<MONTH_INCREMENT_ICON />
								</button>

								{/* @ts-ignore */}
								<Show when={props.showYearButtons}>
									<button ref={yearIncrBtn} onClick={yearIncrement} onKeyDown={handleYearIncrKeyDown}>
										<YEAR_INCREMENT_ICON />
									</button>
								</Show>
							</CalendarButtonGroup>
						...
						<CalendarGrid theme={props.theme}>
							...
							<For each={getWeekdays(props.locale, "narrow")}>
								{weekday => (<WeekdayCell theme={props.theme}>
										{weekday}
									</WeekdayCell>)}
							</For>
							{/* @ts-ignore */}
            ...
            const cellElement = (<CalendarCell theme={props.theme} ref={cellRef} id={idMaker()} class="calendar-cell" color={props.color} onClick={e => handleCellClick(d)} onKeyDown={e => handleCellKeyDown(e, d)} isVisible={isCurrentMonth(d.date, shownDate())} isSelected={!!props.value &&
                    isSameDate(d.date, props.value)} disabled={disabled}>
